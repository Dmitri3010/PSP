...
bool getServerByName(char* name, char* call, struct sockaddr* from, int* flen) // имя пк, позывной сервера, указатель на sockaddr_in, указатель на размер sockaddr_in
{
	try
	{
		SOCKET bS;
		...
		int optval = 1;
		// дескриптор сокета, уровень режима, режим сокета, значение режима,длина буфера (so_broadcast позволяет исп. innadr_broadcast
		...
		//функция для получения информации о хосте по его символическому  имени 
